...
class Questions_Model extends Model {
    ...
    public function getAllQuestions($params = array("category" => "", "tag" => "")) {  //default values of cat and tag might be better to be changed to null
        if (isset($params["getCount"]) and $params["getCount"] != 1) {
            ...
            if ($params["category"] == "" && $params["tag"] == "") {
                $questions = $this->db->select("SELECT count(A.id) as count "
                        . "FROM questions as A "
                        . "INNER JOIN users as B ON A.creator_id = B.userid");
                return intval($questions[0]["count"]);
            } elseif ($params["category"] == "") {
                $questions = $this->db->select("SELECT count(A.id) as count "
                        . "FROM questions as A INNER JOIN users as B ON A.creator_id = B.userid "
                        . "INNER JOIN tags_questions C ON A.id = C.question_id "
                        . "INNER JOIN tags D ON C.tag_id = D.tag_id "
                        . "WHERE D.tag_id = (:tag)", array(':tag' => $params["tag"]));
                return intval($questions[0]["count"]);
            } elseif ($params["tag"] == "") {
                $questions = $this->db->select("SELECT count(A.id) as count "
                        . "FROM questions as A INNER JOIN users as B ON A.creator_id = B.userid "
    ...
    public function getAllTags() {
        $tags = $this->db->select("SELECT TEMP.tag_id, TEMP.tag_name "
                . "FROM (SELECT A.tag_id as tag_id, A.tag_name, COUNT(question_id) as orderer "
                . "FROM tags A INNER JOIN tags_questions B ON A.tag_id = B.tag_id "
                . "INNER JOIN questions C ON B.question_id = C.id GROUP BY A.tag_id, A.tag_name) as TEMP "
    ...
    public function votePlus($userid, $questionId, $answerId) {
        ...
        $this->db->update("questions", array("score" => $initialScore[0]["score"],
                ), "id = " . $questionId);

        $this->db->insert("votes", array("user_voting" => $userid,
    ...
    public function checkIfVoted($voterId, $questionId = 0, $answerId = 0) {
        if ($questionId != 0) {
            $result = $this->db->select("SELECT COUNT(*) as counter "
                    . "FROM votes A INNER JOIN questions B ON B.id = A.question_id_voted "
                    . "WHERE B.id = :qId AND A.user_voting = :voterId"
                    , array(':qId' => $questionId, ':voterId' => $voterId));
            return $result[0]['counter'];
        } else {
            $result = $this->db->select("SELECT COUNT(*) as counter "
                    . "FROM votes A INNER JOIN answers B ON B.id = A.answer_id_voted "
                    . "WHERE B.id = :aId AND A.user_voting = :voterId"
...
// private functions. You can`t call them from outside of this model
    private function numberOfQuestionsInCategory($categoryId) {
        $numberOfQuestions = $this->db->select("SELECT COUNT(*) as counter "
                . "FROM questions WHERE category_id = :cat_id", array(':cat_id' => $categoryId));
    ...
    private function getLatestAnswer($questionId) {
        $result = $this->db->select("SELECT B.userid, B.role, B.username, A.create_date, B.avatar "
                . "FROM questions as A "
                . "INNER JOIN users B ON A.creator_id = B.userid "
    ...
    private function getTagsForEachQuestion($questionId) {
        $result = $this->db->select("SELECT distinct A.tag_id, A.tag_name "
                . "FROM tags A "
                . "INNER JOIN tags_questions B ON A.tag_id = B.tag_id "
                . "WHERE B.question_id = :qId", array(':qId' => $questionId));
        return $result;
    }
